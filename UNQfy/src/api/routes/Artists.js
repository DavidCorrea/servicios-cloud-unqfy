...
router.get("/:id", (req, res, next) => {
  ...
  try{
    ...
    res.status(200).send(artist.serialize({ deep: true }));
  } catch(err) {
    next(err);
  }
});
...
router.get("/", (req, res, next) => {
  ...
  try{
    ...
    res.status(200).send(artists.map(artist => artist.serialize({ deep: true })));
  } catch(err) {
    next(err);
  }
});
...
router.post("/", (req, res, next) => {
  ...
  try{
    ...
    let createdArtist = unqfy.addArtist(artist);
    res.status(201).send(createdArtist.serialize({ deep: true }));
  } catch(err){
    next(err);
  }
});
